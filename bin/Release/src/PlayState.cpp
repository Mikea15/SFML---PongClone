...
void PlayState::OnUpdate(MainGame *Game)
{
    ...
    if( mBallAlive == 1)
    {
        if( ( Game->mAppWindow.GetInput().IsKeyDown(sf::Key::S) ) && ( ( mPad1.GetPosition().y + mPad1.GetSize().y ) <= Game->mAppWindow.GetHeight() ) )
            mPad1.Move(0 , mPadSpeed );
        if( ( Game->mAppWindow.GetInput().IsKeyDown(sf::Key::W) ) && ( mPad1.GetPosition().y >= 0 ) )
            mPad1.Move(0,  -mPadSpeed );
        if( ( Game->mAppWindow.GetInput().IsKeyDown(sf::Key::Down) ) && ( ( mPad2.GetPosition().y + mPad2.GetSize().y ) <= Game->mAppWindow.GetHeight() ) )
            mPad2.Move(0,  mPadSpeed );
        if( ( Game->mAppWindow.GetInput().IsKeyDown(sf::Key::Up) ) && ( mPad2.GetPosition().y >= 0 ) )
            mPad2.Move(0,  -mPadSpeed );
...
void PlayState::Draw(MainGame *Game)
{
